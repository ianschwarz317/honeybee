...
const pimsPartners = [
  { name: 'Avimark', desc: 'Most widely used PIMS in U.S. general practice', status: 'Design Partner Sought', color: '#0071E3' },
  { name: 'Cornerstone', desc: 'IDEXX\'s flagship clinic management system', status: 'Design Partner Sought', color: '#34C759' },
  { name: 'ezyVet', desc: 'Cloud-based, popular with modern practices', status: 'Design Partner Sought', color: '#FF9F0A' },
  { name: 'Shepherd', desc: 'Next-gen PIMS built for high-volume clinics', status: 'Design Partner Sought', color: '#AF52DE' },
  { name: 'ImproMed', desc: 'Enterprise solution for multi-location groups', status: 'Design Partner Sought', color: '#FF3B30' },
  { name: 'VetPace', desc: 'Australian-origin cloud PIMS, growing U.S. footprint', status: 'Design Partner Sought', color: '#5856D6' },
]

const flowSteps = [
  { icon: '📱', label: 'Phone / Scanner', sub: 'NFC tap or 134.2 kHz read', side: 'left' },
  { icon: '🐝', label: 'Honeybee Platform', sub: 'Identity, records & routing layer', highlight: true },
  { icon: '🏥', label: 'PIMS System', sub: 'Existing clinic software', side: 'right' },
]
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <main style={{ flex: 1, padding: 28, overflow: 'auto' }}>
          ...
          <div className="card" style={{ padding: '28px 32px', marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, marginBottom: 32, flexWrap: 'wrap' }}>
              ...
              <div style={{ textAlign: 'center', width: 130 }}>
                <div style={{ width: 64, height: 64, borderRadius: 16, background: 'var(--surface-2)', border: '2px solid var(--border-strong)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, margin: '0 auto 10px' }}>📱</div>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 3 }}>Scanner / Phone</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.4 }}>NFC tap or ISO 11784/5 reader</div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: 80, alignItems: 'center' }}>
                {[['records sync', '▶'], ['owner alerts', '▶']].map(([label, arrow]) => (
                  <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ height: 2, width: 60, background: 'var(--border-strong)', position: 'relative' }}>
                      <div style={{ position: 'absolute', right: -5, top: -4, fontSize: 10, color: 'var(--text-tertiary)' }}>{arrow}</div>
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
              {[
                { icon: '🔄', title: 'Bi-directional sync', desc: 'Records created in your PIMS automatically appear in Honeybee and in the owner\'s app.' },
          ...
          {/* PIMS widget mockup */}
          <div className="card" style={{ padding: '28px 32px', marginBottom: 24 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>What integration looks like in your PIMS</h2>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 24 }}>
              The Honeybee panel surfaces directly within your existing clinic software — no workflow changes required.
            </p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', minHeight: 360 }}>
                ...
                <div style={{ background: '#FAFAFA', borderLeft: '1px solid #E0E0E0', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ flex: 1, padding: '14px 14px 0' }}>
                    {/* Chip status */}
                    <div style={{ background: 'white', borderRadius: 9, padding: '12px', marginBottom: 10, border: '1px solid #EBEBEB' }}>
                    ...
                    <div style={{ background: 'white', borderRadius: 9, padding: '12px', marginBottom: 10, border: '1px solid #EBEBEB' }}>
                      ...
                      <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.5 }}>Healthy 6yr male Golden Retriever. Vaccines current to Apr 2029. Monthly Heartgard. Dental cleaning recommended.</div>
                    </div>

                    {/* Records sync status */}
                    <div style={{ background: 'white', borderRadius: 9, padding: '12px', border: '1px solid #EBEBEB', marginBottom: 14 }}>
                      ...
                      {[['Last synced', '2 min ago'], ['Records synced', '4'], ['Owner notified', 'Yes']].map(([l,v]) => (
          ...
          <div className="card" style={{ padding: '28px 32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>We are actively seeking design partners across major PIMS platforms. Early partners will co-shape the integration specification.</p>
              </div>
              <button className="btn-primary" style={{ fontSize: 14, whiteSpace: 'nowrap', flexShrink: 0 }}>Become a Design Partner</button>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
              {pimsPartners.map(({ name, desc, status, color }) => (
                <div key={name} style={{ border: '1px solid var(--border-strong)', borderRadius: 12, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: color }}>
                      {name[0]}
                    </div>
                    <span style={{ fontSize: 11, color: color, background: color + '15', borderRadius: 100, padding: '3px 10px', fontWeight: 600 }}>Design Partner Sought</span>
                  </div>
                  <div>
                  ...
                  <button style={{ marginTop: 4, border: '1px solid var(--border-strong)', borderRadius: 7, padding: '8px', fontSize: 13, background: 'var(--surface)', color: 'var(--text-primary)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>Express Interest →</button>
                </div>
              ))}
            </div>
          </div>
